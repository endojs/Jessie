...
const makeJSON5 = peg => {
  ...
  try {
    unicodeCombiningMarkRegExp = RegExp(String.raw`\p{Mn}|\p{Mc}`, 'u');
  } catch (_e) {
  ...
  try {
    unicodeDigitRegExp = RegExp(String.raw`\p{Nd}`, 'u');
  } catch (_e) {
  ...
  try {
    unicodeConnectorPunctuationRegExp = RegExp(String.raw`\p{Pc}`, 'u');
  } catch (_e) {
