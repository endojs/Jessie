...
const ERROR = (self, _pos) => {
  ...
  let found = FIND(self.template, last);
  let tokStr;
  if (Array.isArray(found)) {
    tokStr = `at ${last} ${makeTokStr(self, found)}`;
  } else {
    found = FIND(self.template, last - 1);
    tokStr = `unexpected EOF after ${makeTokStr(self, found)}`;
  }

  const failStr =
    fails.length === 0 ? `stuck` : `looking for ${fails.join(', ')}`;
  const { sources } = self.template;
  const raw = self.template.raw;
  const focus = Array.isArray(found) ? found[0] : raw.length;
  const begin = Math.max(0, focus - 5);
  const end = Math.min(focus + 5, raw.length);
  console.info(`\
-------template--------
${raw.slice(begin, end).reduce((prior, r, i) => {
  const cur = begin + i;
  if (cur === focus) {
    prior += `>>> `;
  } else {
...
pos = ${posSrc};`;
  ...
  if (a !== b) {
    console.info(
      `// boot-pegast.js - AUTOMATICALLY GENERATED by boot-peg.js\nexport default harden(${b});`,
    );
