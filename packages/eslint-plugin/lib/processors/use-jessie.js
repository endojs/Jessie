...
const USE_JESSIE_REGEXP = /^('use\s+jessie'|"use\s+jessie"|import\s+('@jessie.js\/transform-this-module'|"jessie.js\/transform-this-module"))/;
...
function isJessie(text) {
  ...
  if (pos >= 0) {
    if (USE_JESSIE_REGEXP.test(text.substr(pos))) {
      return true;
    }
  }
  return false;
